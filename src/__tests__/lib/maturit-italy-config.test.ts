...
import { EXAM_CONFIG } from "@/lib/maturit-italy/config";

describe("Maturità Italy Config", () => {
  it("has correct exam ID", () => {
    expect(EXAM_CONFIG.examId).toBe("EXAM115");
  });

  it("has correct slug", () => {
    expect(EXAM_CONFIG.slug).toBe("maturit-italy");
  });
